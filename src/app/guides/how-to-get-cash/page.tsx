...
const steps: GuideStep[] = [
  ...
  { title: "Use code Cash to remove a bottleneck", body: <p>If both September 5, 2026 reported codes redeem, the reported total is 100,000 Cash. Use it for income, Barracks access, a needed squad, or recovery—not a fixed list copied from an older patch.</p> },
...
const faq = [
  ...
  { q: "How much Cash do the current codes give?", a: "Sources checked September 5, 2026 report 50,000 Cash for BONUS and 50,000 Cash for JOINED. This site has not independently redeemed them." },
...
export default function CashGuidePage() {
  return (
    <GuideArticle
      eyebrow="Economy guide • reviewed September 6, 2026"
      ...
      status={<p className="text-sm text-white/70"><strong>Current code status:</strong> BONUS and JOINED are cross-source reported. <strong>Exact income rates:</strong> not verified.</p>}
      beforeSteps={<section className="content-card"><h2>Current Cash sources</h2><div className="mt-4 overflow-x-auto"><table><thead><tr><th>Source</th><th>Supported</th><th>Not yet supported</th></tr></thead><tbody>
        ...
        <tr><td>BONUS / JOINED</td><td>Multiple sources report 50,000 Cash each on September 5, 2026</td><td>Independent live redemption here</td></tr>
      ...
      faq={faq}
      sourceNotes={<><aside><strong>Purchase decisions:</strong> stabilize income; unlock required Barracks access; retain recovery Cash; verify the next objective. For Rebirth, read the <Link href="/guides/rebirth/" className="font-bold text-cyan-300">current-prompt checklist</Link>.</aside><aside><strong>Evidence boundary:</strong> No Cash-per-minute or ROI values are published. The +10% Income result is tied to an August 31, 2026 Tent Camp observation; all-location rewards and stacking remain unconfirmed. See the distinct <Link href="/wiki/base-economy/" className="font-bold text-cyan-300">base-system record</Link>.</aside></>}

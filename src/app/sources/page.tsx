...
export default function SourcesPage() {
  return (
    <main className="mx-auto max-w-4xl px-4 py-10">
      ...
      <PageIntro eyebrow="Editorial method • updated September 12, 2026 (UTC)" title="Sources and Editorial Method" description="Military Army Tycoon Wiki covers Battle Action's Roblox experience at Place ID 95409544559668. It is not operated by Roblox or Battle Action. The goal is to answer player tasks without mixing in similarly named games." />
      ...
      <section className="content-card"><h2>How conflicts are handled</h2><p>A guide omitting a requirement is not the same as a guide denying it. We record whether a source explicitly shows a condition, explicitly rejects it, or simply does not mention it. For code redemption, the live account message is more specific than an incomplete written walkthrough.</p><p>Several pages repeating one video still count as one underlying source. A readable third-party still can support the text visible in that image, but does not establish the original recording date, the full sequence or the current game’s behavior.</p><p>An earlier draft on this website is not independent verification of a game mechanic. We trace important claims to the original record. Publicly accessible artwork or screenshots are not automatically licensed for copying; credit alone does not grant permission.</p></section>

    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[color:var(--bg)]/92 backdrop-blur">
      <div className="mx-auto flex h-16 w-full max-w-7xl min-w-0 items-center justify-between gap-4 px-4">
        <Link href="/" className="flex min-w-0 flex-1 items-center gap-3 lg:flex-none" onClick={() => setMobileOpen(false)}>
          <Image src="/game-icon.png" alt={`${siteConfig.name} game icon`} width={40} height={40} className="h-10 w-10 rounded-lg border border-white/10 bg-black/30" />
          <span className="site-brand-label truncate text-lg font-extrabold text-white">{siteConfig.name}</span>
        </Link>

        <nav className="desktop-primary-nav items-center gap-1" aria-label="Primary navigation">
          {siteConfig.navGroups.map((group) => {
            ...
            return (
              <div key={group.label} className="group relative">
                ...
                >
                  {group.label}
                  {group.items.length > 0 ? <ChevronDown aria-hidden="true" size={14} strokeWidth={2.4} /> : null}
                </Link>
                {group.items.length > 0 ? (
                  <div className="invisible absolute left-0 top-full w-72 pt-2 opacity-0 transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
                    <div className="nav-dropdown rounded-lg p-2 shadow-2xl">
        ...
        <div className="desktop-header-actions items-center gap-2">
          <a href={gameConfig.dataSources.officialGameUrl} target="_blank" rel="noreferrer" className="button-primary button-small">
            Play on Roblox <ExternalLink aria-hidden="true" size={15} />
          </a>
        </div>
        ...
        >
          {mobileOpen ? <X aria-hidden="true" size={21} /> : <Menu aria-hidden="true" size={22} />}
        </button>
      </div>
      ...
      {mobileOpen ? (
        <nav id="mobile-site-nav" className="mobile-site-nav border-t border-white/10 bg-[#111113]" aria-label="Mobile navigation">
          <div className="mx-auto grid max-w-7xl gap-3 px-4 py-4">
            <Link href="/" onClick={() => setMobileOpen(false)} className="mobile-home-link">
              Home
            </Link>
            <a
